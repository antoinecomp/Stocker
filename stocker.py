...
'''
Stockings is a modified version of Stocker by Will Koehrsen. 
Stocker is a Python class-based tool for stock analysis and prediction on WIKI database on Quandl by default 
and a list of all 3100 tickers in this database.
Stockings allows us to add our own source from an API given as an URL, and to better .
'''

__author__ = "Will Koehrsen, modified by Antoine Coppin"
__copyright__ = "All Rights Reserved"
__credits__ = ["Will Koehrsen", "Antoine Coppin"]
__license__ = "All Rights Reserved"
__version__ = "0.0a2019" # following PEP 440 -- Version Identification and Dependency Specification
__maintainer__ = "Antoine Coppin" # until February 14th
__email__ = "antoine.coppin@luxurynsight.com"
...
# to restore default parameters in prophet model
import matplotlib

# for calling api and storing it
import json
import requests
...
class Stocker():
    ...
    def __init__(self, ticker, exchange='WIKI', source = None):
        ...
        try:
            if source == None :
                ...
                # WARNING : until now we 
                url = source
                # source example = 'https://finance.services.appex.bing.com/Market.svc/ChartAndQuotes?symbols=160.1.KER.PAR&chartType=5y'
                resp = requests.get(url=url)
                prices = resp.json()
                # WARNING If the source is different we consider it to be of MSN finance format
                stock = pd.DataFrame(prices[0]['Chart']['Series'])

                # Corregir origen de timestamps
                origen = stock.loc[0, "T"]
                stock.loc[1:, "T"] = stock.loc[1:, "T"] + origen

                # Convertir a datetime
                stock["Date"] = pd.to_datetime(stock["T"], unit="m", origin=pd.Timestamp('2000-01-01'))
        ...
        # WARNING : until now 
            ...
            stock['Open']=stock['P'].shift(1)
            stock = stock.rename(columns={'P':'Close','fecha':'Date'})
    ...
    def evaluate_prediction(self, start_date=None, end_date=None, nshares = None):
        ...
        if not nshares:
            ...
            #ax.set_xlim([datetime.date.today() - DD, datetime.date.today() + DD])
